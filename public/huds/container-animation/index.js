...
function fillPlayer(player, nr, side, max) {
    ...
    if (oldhp != statistics.health) {
        ...
        if (statistics.health == 0) {
            $top.find("#bar_username").addClass("dead");   
        }
        else {
            $top.find("#bar_username").removeClass("dead")
        }
    }
    //$top.find(".hp_bar").css("background", gradient);
    /*$top.find(".hp_bar").animate({
        float:side,
        background:"rgba(0,0,0,0) " + (100-statistics.health) + "%, " + health_color + " " + (100-statistics.health) + "%)"
    }, 500 );
    */
...
function updatePage(data) {
    ...
    if (observed.steamid == 1 || !observed) {
        if (container.attr('animating') == "false") {
            container.attr('animating', "true").hide("drop", {direction:"down"}, 500, function () {
                container.attr('animating', "false")
            })
        }
    } else if (observed) {
        menu = (data.info.player.activity == "menu");
        if (!menu && $("#player-container").attr('steamid') != observed.steamid) {
            container.attr('steamid',observed.steamid)
            if (container.attr('animating') == "false") {
                container.attr('animating', "true").show("drop", {direction:"down"}, 500, function () {
                    container.attr('animating', "false")
                })
            }
        }
